...
def main():
    ...
    with dot.subgraph(name="cluster_l0") as sg:
        sg.attr(label="Level 0 — Field Devices", style="filled",
                fillcolor="#EBF5FB", color="#1A5276")
    ...
    with dot.subgraph(name="cluster_l1") as sg:
        sg.attr(label="Level 1 — Basic Control", style="filled",
                fillcolor="#E9F7EF", color="#1E8449")
        sg.node("rtu", "RTU-CTRL-03\n(Windows)", shape="box", style="filled", fillcolor="#A9DFBF")

    with dot.subgraph(name="cluster_l2") as sg:
        sg.attr(label="Level 2 — Supervisory Control", style="filled",
    ...
    with dot.subgraph(name="cluster_l3") as sg:
        sg.attr(label="Level 3 — Operations Management", style="filled",
                fillcolor="#FDEDEC", color="#922B21")
    ...
    with dot.subgraph(name="cluster_detect") as sg:
        ...
        sg.node("winlog", "Windows\nEvent Log\nCollector", shape="cylinder",
                style="filled", fillcolor="#D7BDE2")
        sg.node("feat_eng", "Feature\nExtraction\nEngine", shape="diamond",
                style="filled", fillcolor="#C39BD3")
        sg.node("clf", "Random\nForest\nClassifier", shape="ellipse",
                style="filled", fillcolor="#A569BD", fontcolor="white")
        sg.node("siem", "SIEM /\nAlert", shape="doubleoctagon",
                style="filled", fillcolor="#6C3483", fontcolor="white")

    dot.node("attacker", "Adversary\n(APT)", shape="invtriangle",
